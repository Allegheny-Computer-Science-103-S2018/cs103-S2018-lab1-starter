...
if [ "$CHECK" = true ]; then
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/www/ \
                                     --checkfiles answers.md --fragments "##" --fragmentcounts 4
  determine_exit_code $?
  ...
  determine_exit_code $?
  # --> GatorGrader CHECK: the reflection contains at least 4 paragraphs with 3 sentences each
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/www --checkfiles answers.md --paragraphs 4 --sentences 3
  determine_exit_code $?
  # --> GatorGrader CHECK: the repository contains at least five commits (update to current + 5)
  python3 gatorgrader/gatorgrader.py --nowelcome --commits 13
  determine_exit_code $?
  echo ""
